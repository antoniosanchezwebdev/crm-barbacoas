<div class="container mx-auto">
    <form wire:submit.prevent="update">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <table class="table responsive">
                    ...
                    <tbody>
                        @foreach ($operarios as $operarioIndex => $operario)
                            <tr>
                                <td>{{ $trabajadores->where('id', $operario)->first()->name }}</td>
                                <td><button type="button" class="btn"
                                        style="background-color: #b10510; color: white;"
                                        wire:click.prevent="deleteTrabajador('{{ $operarioIndex }}')">X</button></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <div wire:key='hola' class="mb-3 row d-flex align-items-center" x-data=""
                    ...
                    <div class="col-sm-2">
                        <button type="button" class="btn btn-tab" wire:click="addTrabajador">Añadir
                            operario</button>
                    </div>
                </div>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table responsive">
                    ...
                    <tbody>
                        @for ($i = 0; $i < count($this->trabajos_realizarAux); $i++)
                            <tr>
                                <td>
                                    <input type="text" wire:model="trabajos_realizarAux.{{ $i }}.titulo"
                                ...
                                <td>
                                    <input type="text"
                                        wire:model="trabajos_realizarAux.{{ $i }}.descripcion"
                                        class="form-control" name="descripcionAux_{{ $i }}"
                                        id="descripcionAux_{{ $i }}" disabled>
                                </td>

                                <td>
                                    <input type="number"
                                        wire:model="trabajos_realizarAux.{{ $i }}.horas_estimadas"
                                        class="form-control" name="horas_estimadasAux_{{ $i }}"
                                        id="horas_estimadasAux_{{ $i }}" disabled>
                                </td>
                                <td>
                                    <input type="number"
                                        wire:model="trabajos_realizarAux.{{ $i }}.precio"
                                        class="form-control" name="precioAux_{{ $i }}"
                                        id="precioAux_{{ $i }}" wire:change='generarSubtotal' disabled>
                                </td>
                                <td> <button type="button" class="btn"
                                    style="background-color: #b10510; color: white;"
                                    wire:click.prevent="alertaTrabajo">X</button></td>
                            </tr>
                        @endfor
        ...
        <br>
        <div>
            <button type="submit" class="btn btn-tab w-100 mb-5">Crear
                parte de
                trabajo</button>
        </div>
    </form>
